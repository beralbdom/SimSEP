function [medidores, Nmed, mat_med]=ler_medidores(med_file)
...
% dp            desvio-padrão (VARIÂNCIA na verdade)
...
mat_med = zeros(39,12); % matriz auxiliar que irá receber em cada coluna os medidores
...
mat_med = [transp(medidores.num) transp(medidores.de) transp(medidores.para) transp(medidores.circ) transp(medidores.tipo) transp(medidores.PMU_num) transp(medidores.ok) transp(medidores.acc) transp(medidores.fs) transp(medidores.dp) transp(medidores.ref) transp(medidores.leitura)];
