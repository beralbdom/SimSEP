function [Caso, Barra, Ramo]=escrever_sistema_v2(matriz_aux , matriz_carga_MVAR)
...
% ---------------------------------------------------------------------- %
% ------------------- Inicio do escrever_sistema ----------------------- %
% ---------------------------------------------------------------------- %

% Gera 24 arquivos '.cdf' contendo as informações da potência ativa por hora de cada uma das 14 barras

for jj = 1:24;
    filen=strcat('ieee14-',int2str(jj),'.txt');
    BUS = 'Bus';
    kV = 'kV';
    Point = '.';
    Break = '-999';
    ...
    for ii = 1:14
        if ii <= 9;
            fprintf(fidr, format1, Barra.num(ii), BUS, Barra.num(ii), Barra.T(ii), kV, Barra.area(ii), Barra.zona(ii), Barra.tipo(ii), Barra.tensao(ii), Barra.angulo(ii), matriz_aux(ii,jj), matriz_carga_MVAR(ii,jj), Barra.ger_MW(ii), Barra.ger_MVAR(ii), Barra.kVBase(ii), Barra.tensao_inic(ii), Barra.lim_max(ii), Barra.lim_min(ii), Barra.g(ii), Barra.b(ii), Barra.ctrl_rem(ii) );    
        end
        if ii > 9 & ii <= 14;
            fprintf(fidr, format2, Barra.num(ii), BUS, Barra.num(ii), Barra.T(ii), kV, Barra.area(ii), Barra.zona(ii), Barra.tipo(ii), Barra.tensao(ii), Barra.angulo(ii), matriz_aux(ii,jj), matriz_carga_MVAR(ii,jj), Barra.ger_MW(ii), Barra.ger_MVAR(ii), Barra.kVBase(ii), Barra.tensao_inic(ii), Barra.lim_max(ii), Barra.lim_min(ii), Barra.g(ii), Barra.b(ii), Barra.ctrl_rem(ii) );
        end
    end
    fprintf(fidr,'-999 \r\n');
