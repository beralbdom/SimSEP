function [mat_fluxo, voltage, ang] = FLow2(M,N)
...
%%%%%%%%%%%%%%%%%%%% Armazenamento das iterações %%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  ang = 180*transp(t)/pi;     % ----------- linha inclusa by JACSON ----------- %

  
  for jj=1:nv
    v(indice(jj+nt))=v(indice(jj+nt))+dv(jj);
  end
  voltage = transp(v);        % ----------- linha inclusa by JACSON ----------- %
...
mat_fluxo = fluxo(1:NR(1) , 1:10); % ----------- linha inclusa by JACSON | mat_fluxo desconsidera os dados refentes ao I cartesiano ----------- %
