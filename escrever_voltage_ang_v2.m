function [Caso, Barra, Ramo]=escrever_voltage_ang_v2(matriz_aux, matriz_carga_MVAR, mat_voltage,col_ang)
...
%             fprintf('Barra de Referencia: %d\n', Barra.num(ibus,1));
    ...
    Barra.angulo(ibus,1) = (str2double(tline(34:40)))*180/pi;              % ESCREVENDO O ÂNGULO EM GRAUS NOS ARQUIVOS "ieee-x.txt"
    if Barra.tipo(ibus,1) == 3
...
% Nos 24 arquivos formato '.cdf' gerados, acrescenta a informação de Tensão e Ângulo;
% Gera 24 novos arquivos formato '.cdf'com essas informações;
% Os arquivos têm nome 'Relatorio-x.txt'.

for jj = 1:24;
    filen=strcat('Relatorio-',int2str(jj),'.txt');
    
    celldata(jj) = cellstr(filen);
    arq_relat = celldata;
    ...
    for ii = 1:14
        if ii <= 9;
            fprintf(fidr, format1, Barra.num(ii), BUS, Barra.num(ii), Barra.T(ii), kV, Barra.area(ii), Barra.zona(ii), Barra.tipo(ii), mat_voltage(ii,jj), col_ang(ii,jj), matriz_aux(ii,jj), matriz_carga_MVAR(ii,jj), Barra.ger_MW(ii), Barra.ger_MVAR(ii), Barra.kVBase(ii), Barra.tensao_inic(ii), Barra.lim_max(ii), Barra.lim_min(ii), Barra.g(ii), Barra.b(ii), Barra.ctrl_rem(ii) );    
        end
        if ii > 9 & ii <= 14;
            fprintf(fidr, format2, Barra.num(ii), BUS, Barra.num(ii), Barra.T(ii), kV, Barra.area(ii), Barra.zona(ii), Barra.tipo(ii), mat_voltage(ii,jj), col_ang(ii,jj), matriz_aux(ii,jj), matriz_carga_MVAR(ii,jj), Barra.ger_MW(ii), Barra.ger_MVAR(ii), Barra.kVBase(ii), Barra.tensao_inic(ii), Barra.lim_max(ii), Barra.lim_min(ii), Barra.g(ii), Barra.b(ii), Barra.ctrl_rem(ii) );
        end
    end
    fprintf(fidr,'-999 \r\n');
    ...
    [nlin,ncol] = size(arq_relat);

    for kk = 1:24
        copyfile (arq_relat{nlin,kk}, 'Estimador_Hibrido\sist');
    end
return;